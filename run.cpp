...
typedef struct _Event{
    ...
    int event_type;         // Event Type (イベントタイプ)
...
void register_event(double s_time, int e_type, int pkt_id){
...
int main(){
    ...
    register_event(START_TIME, static_cast<int>(EVENT_TYPE::ARRIVAL), global_pkt_id++);
    register_event(FINISH_TIME, static_cast<int>(EVENT_TYPE::TERMINATOR), -1);

    while (1)
    {
        if (event_table.top().event_type == static_cast<int>(EVENT_TYPE::TERMINATOR)){
#if DEBUG
        ...
        else if (event_table.top().event_type == static_cast<int>(EVENT_TYPE::ARRIVAL)){
#if DEBUG
            cout << "  event_type = Arrival" << endl;
#endif
            register_event(sim_time + exp_rand(), static_cast<int>(EVENT_TYPE::ARRIVAL), global_pkt_id);

            if (pkt_queue.size() == 0){
                register_event(sim_time + exp_rand(), static_cast<int>(EVENT_TYPE::DEPARTURE), event_table.top().packet_id);
            }            

            pkt_queue.push(global_pkt_id++);

        }
        else if (event_table.top().event_type == static_cast<int>(EVENT_TYPE::DEPARTURE)){
#if DEBUG
            cout << "  event_type = Departure" << endl;
#endif
            if (pkt_queue.size() > 1){
                register_event(sim_time + exp_rand(), static_cast<int>(EVENT_TYPE::DEPARTURE), pkt_queue.front());
            }            
